...
if __name__ == "__main__":
  ...
  ax.fill_between(zs, np.zeros_like(zs), kernel / kernel.max(), alpha=0.2, color='navy', label=r'$W^{\kappa}$')
  ...
  for k, ilim in enumerate(ilims):
    try:
      pl.plot(zs, lsst_chang_pz(zs, ilims[ilim]), label=ilim, lw=1.5, c=colors[k])
