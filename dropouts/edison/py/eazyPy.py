...
def plotExampleSED(idx=0, pdfname='out.pdf', MAIN_OUTPUT_FILE = 'photz', OUTPUT_DIRECTORY = 'OUTPUT', CACHE_FILE = 'Same', lrange = [3.e3, 8.e4], axes=None,\
                   individual_templates=False, fnu=False, show_pz=True, snlim=2, scale_flambda=1.e-17, show_rest=False, plotit=False):
    """
    ...
    if plotit:
      ...
      if individual_templates:
        ...
        ax.plot(lambdaz / xrest, temp_sed, linewidth=1.5, color='blue', alpha=0.4, zorder = -3, label='Template')
    ...
    vs   = (1.e+10 / lambdaz) * const.c.to('m/s').value                      ## Redshifted wavelengths.                                                       
    ...
    filters     = prep_filters(['LSST', 'VIDEO'])
    ...
    vs, nFv     = magAB(vs, mag = fivesig)
    nFv         = np.random.normal(0.0, nFv / 5., len(vs))

    result      = [zout['id'][idx], zout['z_peak'][idx]] 

    for noise in [np.zeros_like(nFv), nFv]:
        mags    = get_appmags(vs, Fv + noise, filters, printit = True)
        colors  = get_colors(mags, LBGCOLORS, fname = None)
        
        print 
    
        for color in LBGCOLORS:
            result += [colors[color]]

    if plotit:
      for i, band in enumerate(filters.keys()):
        ax.plot(filters[band]['ls'], max(fobs) * filters[band]['Ts'], label = filters[band]['ppkey'])

      ## Set axis range and titles
      ax.semilogx()
  
      ax.set_xlim(lrange[0], lrange[1])
      ax.set_ylim(-0.5 * max(obs_sed), 1.5 * max(fobs))
    
      ax.set_xlabel(r'$\lambda%s$ [$\AA$]' % (rest_label))
      ax.set_ylabel(r'$F_\lambdba \ [10^{-17} \ \rm{ergs} / s / cm^2 / A]$')

      ax.legend(ncol=1)
    
      ## P(z)
      if (pz is not None) & (show_pz):            
        axp.plot(zgrid, pz, linewidth=1.0, color='orange', alpha=0.9, label=r'$z=%.3lf$' % zout['z_peak'][idx])
      ...
      pylab.savefig(pdfname, bbox_inches='tight')
