...
def colourtrack():    
    ...
    pl.savefig('plots/tracks/%scolour_track_mag_24p5.pdf' % dband, bbox_inches='tight')

def plot_cat():
    cols         = ['id', 'zpeak', 'u-g', 'g-r', 'r-i', 'i-z']

    ## colors    = np.loadtxt('aegis_colors.txt')
    colors       = np.loadtxt('aegis_mag_24p5_colors.txt')
    ...
    if dband == 'g':
        ...
        cax     = plt.scatter(colors[:,3], colors[:,2], c=colors[:,1])

    else:
        raise ValueError("\n\nDropband colour criteria is not available with tracks.\n\n")
