...
def  Raf15_dNdz(droptype='u', field='UVUDF', depth='FULL', dz=0.1):
...
def Raf15_pz(droptype='u', field='UVUDF', depth='FULL', dz=0.1):
    midz, dNdz = Raf15_dNdz(droptype=droptype, field=field, depth=depth, dz=dz)
...
def Raf15_intfrac(droptype='u', field='UVUDF', depth='FULL', dz=0.1):
    midz, dNdz = Raf15_dNdz(droptype=droptype, field=field, depth=depth, dz=dz)
    ...
    for depth, title in zip(['FULL', 'DEFAULTDEPTHS'], ['Full depth', 'Reduced depth']):
      ...
      for ttype, label in zip(['u', 'g', 'BzK', 'Euclid'], [r'$u$-dropouts', r'$g$-dropouts', r'$BzK$', 'All']):
        for field in ['UVUDF']:
          ...
          midz, ps   =  Raf15_pz(droptype=ttype, field=field, depth=depth, dz=dz)
