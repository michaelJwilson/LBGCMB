...
CC
      ...
      common/err/beta,fid,binstep,hess
      common/test/teste1(50,50),teste2(50,50)
C
      LDEBUG = (IDBG(3) .GE. 1)
...
c     J'ajoute pour la contrainte
      if(hess.gt.50.)then
        do i=1,NPAR
         do j=1,NPAR
           P(I,J)=(binstep**2)/2*P(I,J)-binstep**2
         enddo
         P(i,NPAR+1)=-binstep
         P(NPAR+1,i)=-binstep
        enddo
        P(NPAR+1,NPAR+1)=1.d-50
        do i=1,NPAR+1
         write(6,500)(P(I,J),j=1,NPAR+1)
         do j=1,NPAR+1
           teste1(I,J)=P(I,J)
         enddo
        enddo
 500    format(40(f15.5))
      ...
      if(hess.gt.50.)then          
        CALL MNVERT(P,MAXINT,MAXINT,NPAR+1,IFAIL)
        do i=1,NPAR+1
         do j=1,NPAR+1
           teste2(I,J)=P(I,J)
         enddo
      ...
      IF (IFAIL .GT. 0)  THEN
        CALL MNWARN('W','HESSE', 'Matrix inversi on fails.')
        GO TO 390
      ENDIF
C                                        . . . . . . .  calculate  e d m
      EDM = 0.
