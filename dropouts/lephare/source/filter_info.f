...
c      real*8    corr0(nbf),corr1(nbf),corr2(nbf),corr3(nbf)
      ...
      character*4096 vega,sun,filtfile,param,bd17
...
c
      param='-f'
      call get_conf(param,filtfile,test)
...
cc
c      write(*,*) ' Give the filter file name '
c      write(*,*) ' it will search it in $LEPHAREWORK/filt/* '
c      read(*,'(A)')  filtfile
c      filtfile='filter_test.dat'
...
c  reading name, lambda, T(lbda) of filters and defines the WIDTH
        open(1,file=filtfile,status='old',err=56)
        read(1,'(A)') str
...
c  define the width based on a fraction of the Peak level
     ...
     >            rep(i,j+1).ge.(fpic(i)*frac) .and. pass.eq.0 ) then
c
...
c Resampling Delta(lambda) with 1A
     ...
     >                      (lamb(i,j+1)-lamb(i,j))
                     do k=1,IDINT(lamb(i,j+1)-lamb(i,j))+1
                       if ((rep(i,j)+slope*DBLE(k)).ge.
...
c Resampling Delta(lambda) with 1A
                     slope=(rep(i,j+1)-rep(i,j))/
     >                      (lamb(i,j+1)-lamb(i,j))
                     do k=1,IDINT(lamb(i,j+1)-lamb(i,j))
                       if ((rep(i,j)+slope*DBLE(k)).le.
     >                      (fpic(i)*frac) ) then
                          lbsup(i)= lamb(i,j)+DBLE(k) 
                         goto 63
                       endif
                     enddo
 63                  continue
...
c 40   format(A10,2x,f8.3,2x,f8.3,2x,f9.3)
      ...
      STOP
 56    write (6,*) 'File ',filtfile(1:lnblnk(filtfile)),
     >  ' not found -> STOP '
