...
c  libraries
      ...
      integer*4  modext(20)
      integer*4  modlib(inlib),extlib(inlib),reclib(inlib) 
      integer*4  fobs(chisize,nbf,nbf),fobs4(chisize,nbf,nbf)
      real*8     minkcol(chisize,nbf,nbf)
      real*8     ebvlib(inlib)
      real*8     zlib(inlib),agelib(inlib)
      real*8     maglib(nbf,inlib),klib(nbf,inlib),emlib(nbf,inlib)
      real*8     zflib(inlib),maglibf(nbf,inlib)
      real*8     zstep,zmax,dz,h0,om0,l0
      real*8     zf(500),ebv(500)
c  length for phys parameters 50,10000 may be changed here and read_lib prog. 
...
c  FIR Library 
      ...
      real*8         klfir(nbf,inlib)
      real*8         dmagir(5),lumlirb(5)
      integer*4      kbestir(5),sens
      real*8         akir,kcfirb(nbf),magfirb(nbf)
c  FIR Chi 2
...
c  PHYS Library 
      ...
      real*8         libppara(30,maxsize),phys_cont,phys_scale,dmpbest 
c      integer*4      nbuphys,nbsphys,busphys(nbf),bscphys(nbf)  
      real*8         zrecp(500),chipbest
      real*8         fluxphys(nbf)
      real*8         kcorphys(nbf),magphys0(nbf)
      integer*4      zrecpi(500),zrecps(500),nzrecp,nppara(30)
      real*8        ppbest(30),ppmed(30),ppinf(30),ppsup(30)
...
c
      ...
      real*8    zp(nbf),mag(nbf),kcor(nbf),magb(nbf),z,age,zs
      ...
      real*8    zinf,zsup,zmax_gal,ebvmin,ebvmax
...
c global context imposed to all objects
      param='-GLB_CONTEXT'
      call geti_option(param,config,1,paravi,test)
      if (test.eq.1)  gbcont=DBLE(paravi(1))
      if (test.ne.1)  gbcont=-1.d0

c rejected bands 
      param='-FORB_CONTEXT'
      call geti_option(param,config,1,paravi,test)
      if (test.eq.1)  contforb=DBLE(paravi(1))
...
c
      param='-FIR_CONT'
      call geti_option(param,config,1,paravi,test)
      if (test.eq.1)  fir_cont=DBLE(paravi(1))
      if (test.ne.1)  then 
        fir_cont=-1.d0
      endif
c
...
c
      param='-PHYS_CONT'
      call geti_option(param,config,1,paravi,test)
      if (test.eq.1)  phys_cont=DBLE(paravi(1))
      if (test.ne.1)  then 
        phys_cont=-1.d0
      endif
c
...
c
      param='-ADAPT_CONTEXT'
      call geti_option(param,config,1,paravi,test)
      if (test.eq.1)  adcont=DBLE(paravi(1))
...
c         Need at least (!!) filters at Lbda_mean > 7.0 um  
      if (nlibext.ge. 1) then 
          k = 0 
          do i = 1,imag
             if ((flmoy(i)/10000.) .gt. 7.0)  k = k +1
          enddo
          if (k.eq.0) then
            nlibext=0      
            libext(1)='NONE'
            ...
            write(UO,*) '  all filters have Lbda<7um              '
...
c    context for Band used for Chi2
            if (fir_cont.le.0) then      
               if ((flmoy(k)/10000.).ge.7.0)  busfir(k)=1
            else
...
c    context for Band used for scaling 
            if (fir_scale.le.0) then      
               if ((flmoy(k)/10000.).ge. 7.0 .and. busfir(k).eq.1) 
     >               bscfir(k)=1
            else
        ...
        do i = 1, nfir
           do k = 1, imagm
              if (maglfir(k,i) .gt. 90) then 
                 ...
                 if (magtyp(1:1).eq.'A') 
     >              maglfir(k,i)=10**(-0.4*(maglfir(k,i)+48.60))
                 if (magtyp(1:1).eq.'V')
...
c  reject band if error and flux lower than 0 
         ...
         if (bused(k).eq.1) new_cont=new_cont+ DBLE(2**(k-1))
...
c     if (model.ge.0.and.model.le.99) 
c     >       write(*,*) spec,z,model,extil,exti
...
c   rejected models 
     ...
     >     nlib.eq.1 .AND. SNGL(DABS(z-zs)).gt.(SNGL(zstep/2.))) goto 2
...
c   z=0 for Mag abs 
         if (model.ge.0 .and. z.le.1.e-5) irecz0=i   
...
c         Substract the flux which is expected from the emi. lines to the observed flux
                avmago = 0. 
                avmagt = 0. 
                nf = 0 
                do k = 1,imagm
...
c   Analysis of ML function.  Normalizing to peak at ML=1
     ...
     >    .and. zs.ge.0 .and. zs.lt.zmax .and. model.lt.1000) )  goto 4 
c      write(*,*) 'Chimin',chimin(1)
      if (chimin(1).lt.1e9) then
        do k = 1,chimax
          xp(k) = chi(1,k)
          yp(k) = dexp(-0.5*(chi(2,k) - chimin(1)))
          if (yp(k).le.1.d-19) yp(k) = 0.d0
        enddo
cccccccccccccccccccccccccccccccccccccccccccccccccccccc
...
c    REDSHIFT INTERPOLATION  if  ZINTP=Y or ZFIX=Y
      if (zmin(1).ge.0 .and. nlib.eq.1 .and.reclmin(1).ge.1) then
c  Interpolation of Z_best (zmin(1)) via Chi2 curves
         if (zintp(1:1).eq."Y") then
             zbest=zmin(1)
...
c   Z_best replaced by Z_spec if defined 
         if( zfix(1:1).eq.'Y'  .AND.
     >       zs.ge.0 .and. zs.lt.zmax .and. nlib.eq.1) then 
             zbest=zmin(1)
             mod_dist2 = funz(zbest,h0,om0,l0)
             zmin(1)=zs
...
c  recompute the FIR context 
      ...
      do k = 1, imagm
        ...
        if (busfir(k).eq.1) nbufir=nbufir+1
        if (fir_scale.le.0) then      
          if ((flmoy(k)/10000.).ge.7.0.and.busfir(k).eq.1) bscfir(k)=1
        else
          bscfir(k)=bdincl(k-1,fir_scale,imagm-1)
          if (busfir(k).eq.0) bscfir(k)=0
        endif
...
c   CHECH NUMBER OF BANDS USED with Lbda/(1+z)>7um
         cont_fir=0  
         nf_fir=0  
         do k  = 1, imagm 
            abc(k)  = ab(k)
     ...
     >          .and.magb(k).gt.0.and.
     >           (flmoy(k)/10000./(1+zmin(1))).le.15.0) then 
c             remove if stellar does not account for all flux 
             if(sab(k).gt.0 .and. (ab(k)-magb(k)-3*sab(k)).gt.0) then 
                 abc(k)  = ab(k)-magb(k)
                 sabc(k) = dsqrt(sab(k)**2+magb(k)**2)
             else
c          if flux dominated by stellar flux do not used band in FIR measurement  
                 busfir(k)=0
                 bscfir(k)=0
             endif
            endif 
            if (abc(k).gt.0.and.sabc(k).gt.0.and.busfir(k).eq.1
     >          .and. flmoy(k)/(1+zmin(1)) .ge. 70000. ) then
                nf_fir=nf_fir+1
...
c             write(*,*) 'zlib,zspec,dmcor=',zlfir(1),zmin(1),dmcor
             nf    = 0 
             dmir  = 0.
             avmagt= 0. 
             avmago= 0. 
             do k = 1,imagm
               if (abc(k).gt.0.and.sabc(k).gt.0.and.bscfir(k).eq.1)then
                 nf = nf + 1
            avmago=avmago+abc(k)*maglfir(k,j)*dmcor/sabc(k)**2
            avmagt=avmagt+maglfir(k,j)*maglfir(k,j)*dmcor**2/sabc(k)**2
               endif
             enddo
...
cc
       ...
       chi2_fir=chi2_fir+((abc(k)-dmir*maglfir(k,j)*dmcor)/sabc(k))**2
                     nf_fir=nf_fir+1
                endif 
             enddo
c
             if (chi2_fir.lt.chirmin ) then
                ...
                irecfirb= irecfirz0 
c kcor interpolated   
                sens=idnint(dsign(1.d0,(zmin(1)-zlfir(j))))
                akir=dabs((zlfir(j)-zmin(1))/(zlfir(j)-zlfir(j+sens)))
                do k = 1, imagm
                   if(modlfir(j).ne.modlfir(j+sens))  akir=0.d0
                   if(klfir(k,j+sens).gt.90)          akir=0.d0
                   kcfirb(k)=akir*klfir(k,j+sens)+(1-akir)*klfir(k,j)
                enddo
c best magnitude 
                do k = 1,imagm 
                  if (maglfir(k,j).gt.0)
     >         magfirb(k) = -2.5*dlog10(dmir*maglfir(k,j)*dmcor)-48.59
                enddo 
c
             endif
c
...
c
             ...
             do k = 1, imagm
               if (sabc(k).gt.0.and.busfir(k).eq.1) then
                 if (ABS(dlog10(abc(k)/maglfir(k,j)/dmcor)).lt.
     >                                  ABS(dlog10(dmag)))then
                   dmag=  (abc(k)/maglfir(k,j)/dmcor)
                   kbest=j
                 endif 
                 if (ABS(dlog10(abc(k)/maglfir(k,j)/dmcor)).lt.
     >                      ABS(dlog10(dmagir(liblfir(j)))))then
                   dmagir(liblfir(j))=(abc(k)/maglfir(k,j)/dmcor)
                   kbestir(liblfir(j))=j
                 endif 
               endif
             enddo
...
c best magnitude 
                do k = 1,imagm 
                  if (maglfir(k,j).gt.0)
     >       magfirb(k) = -2.5*dlog10(dmag*maglfir(k,j)*dmcor)-48.59
                enddo 
             endif
     ...
     >    fluxphys,kcorphys,magphys0)
          do k = 1, imagm
            if (fluxphys(k).gt.0) then 
              fluxphys(k)= -2.5*dlog10(fluxphys(k))-48.60    
            else
              fluxphys(k)=-99
            endif
...
c
         do k = 1, imagm 
            ...
            if(addem(1:1).eq.'Y') 
     >  magm(k)=-2.5*dlog10(10**(-0.4*(magm(k)+48.6))+emMin(k))-48.6
         enddo   
...
c        
          ...
          if (ndz.gt.1) then
            ...
            do k = 2, ndz  
              call read_spec(gallib,reclb(nb(k)),extilb(nb(z)),
     >          extb(nb(z)),zb(nb(k)),
     >          extic,iext,opal,opat,iopa,
...
c OUTPUT SPECTRA
        ...
        enddo
        if (ndz .eq. 1 .and. nlibext.eq.0) then
          write(7,702) ndz,zmin(1),z68i,z68s,chimin(1),pdz(1),
     >        dmmin(1),imasmin(1),agemin(1),extmin(1)
        elseif (ndz.ge. 1 .and. nlibext.ge.1) then 
          write(7,702) 2,zmin(1),z68i,z68s,chimin(1),pdz(1),
     >        dmmin(1),imasmin(1),agemin(1),extmin(1)
c            
          write(7,705) libfirb,modfirb,zfirb,
     >           lumfirb,lirmed,lirinf,lirsup,
     >           chirmin,nf_fir,dmfirb  
c
        elseif (ndz.gt.1 .and. nlibext.eq.0 ) then                      
          write(7,702) ndz,zmin(1),z68i,z68s,chimin(1),pdz(1),
     >         dmmin(1),imasmin(1),agemin(1),extmin(1),
     >         (zb(nb(k)),chi(2,nb(k)),pdz(k),dmb(nb(k)),
     >         imasb(nb(k)),ageb(nb(k)),extb(nb(k)),k=2,ndz)
        endif
        write(7,703) zmin(2),chimin(2),dmmin(2),imasmin(2),
     >                        imasmin(3),chimin(3)
c
...
c        write(*,*) nspmax,chimax,wmax
     ...
     >                 (wgal(k,i),fgal(k,i),k=1,4)
           if (nlibext.ge.1 .and. nlibphys.eq.0)
     >        write(7,704) val1,val2,wq(i),fq(i),wst(i),fst(i),
     >                 (wgal(k,i),fgal(k,i),k=1,3)
           if (nlibext.eq.0 .and. nlibphys.eq.0)
     >        write(7,704) val1,val2,wq(i),fq(i),wst(i),fst(i),
     >                 (wgal(k,i),fgal(k,i),k=1,2)
...
c
 ...
 702  format(i6,1x,3(f9.3,1x),3(E12.6,1x),1x,i8,1x,E12.6,1x,f9.3,
     > 1x,70(f9.3,1x,3(E12.6,1x),i8,1x,E12.6,1x,f9.3,1x))
 703  format(f9.3,1x,2(E12.6,1x),2(1x,i6),1x,E12.6)
 704  format(f10.3,1x,E12.6,1x,16(E14.8,1x))
 705  format(i6,1x,i8,2x,f5.3,2x,4(f8.3,1x),1x,E12.6,1x,i6,1x,f9.3)
c
c
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
