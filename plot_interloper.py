...
with open('dat/result4interloper_%s.json' % band, 'r') as rfile:
...
pl.axvline(x=fid_sig8, ymin=0.0, ymax=1.0, c='k', label=r'$(\sigma_8)$', alpha=0.6)

for mass_level, color, alpha in zip([0.99, 0.95, 0.68], ['b', 'b', 'b'], [0.2, 0.4, 0.6]):
    plot_ellipse(x_cent = biased_sig8, y_cent = fid_b1, ax = ax, cov = iFisher, mass_level = mass_level,\
                 fill=True, fill_kwargs={'alpha': alpha, 'c': color}, plot_kwargs={'c': color, 'alpha': 0.0})

pl.plot(biased_sig8,    fid_b1,    'w*', markersize=5, label=r'$(\sigma_8, b_1)$')
