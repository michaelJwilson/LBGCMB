...
class EMSpectrum(object):
    ...
    (note: must be a log-base10, i.e., constant-velocity pixel array!) or via the 
    MINWAVE, MAXWAVE and CDELT_KMS arguments.

    In addition, three data files are required: 
    ${DESISIM}/data/recombination_lines.escv,
    ${DESISIM}/data/forbidden_lines.esv,
    ${DESISIM}/data/forbidden_mog.fits.
    ...
    Args:
        ...
        maxwave (float, optional): Maximum value of the output wavelength
    ...
    def __init__(self, minwave=3650.0, maxwave=7075.0, cdelt_kms=20.0, log10wave=None):
        ...
        if log10wave is None:
            cdelt = cdelt_kms/const.c.to('km/s').value/np.log(10)  # pixel size [log-10 A]
    ...
    def spectrum(self, oiiihbeta=None, oiihbeta=None, niihbeta=None,
        ...
        *or* an integrated H-beta line-flux.  Generally an ELG and LRG
        ...
        log10sigma = linesigma/const.c.to('km/s').value/np.log(10) # line-width [log-10 Angstrom]
...
class GALAXY(object):
    ...
    def _blurmatrix(self, vdisp, log=None):
        ...
        blurmatrix = dict()
        for uvv in uvdisp:
            sigma = 1.0 + (self.basewave * uvv / const.c.to('km/s').value)
        ...
        if input_meta is not None:
            ...
            if star_properties is not None:
                ...
                if redshift is None:
                    ...
                    redshift = np.array(vrad) / const.c.to('km/s').value
...
if __name__ == "__main__":
    print("\n\nWelcome to the template constructor.\n\n")
    
    npix    = 1e4
    wave    = np.linspace(np.log10(2.e3), np.log10(1.5e4), npix)

    result  = EMSpectrum(log10wave = wave)
