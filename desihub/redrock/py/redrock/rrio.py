...
def read_template(filename):
    ...
    try:
        ...
        print("\n\nReading template from redrock template directory: %s" % os.getenv('RR_TEMPLATE_DIR'))
    ...
    if rrtype    == 'GALAXY':
        ...
        redshifts = 10. ** np.arange(np.log10(loz), np.log10(hiz), step)
    ...
    elif rrtype.split('-')[0]  == 'STAR':
        ## As low-z star.
        loz       = -0.0100 
        hiz       =  0.0101

        redshifts = np.arange(loz, hiz, 4e-5)
    ...
    elif rrtype in ['Q0_199', 'Q2_198', 'Q3_199', 'Q4_198', 'QX_811']:
        loz       = 1.50
        hiz       = 4.00
    ...
    print("\nLoading %s template with redshift limits: %.3lf < z < %.3lf" % (rrtype, loz, hiz))
...
def write_zscan(filename, zscan, zfit, clobber=False):
    ...
    The nested dictionary structure of results is mapped 
    into a nested group structure of the HDF5 file:
