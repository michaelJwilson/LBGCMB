...
from    dataobj        import  (MultiprocessingSharedSpectrum, SimpleSpectrum, MPISharedTargets, Target)
...
def mpi_calc_zchi2_targets(redshifts, targets, template, verbose=False, comm=None):
    '''
...
def calc_zchi2_targets(redshifts, targets, template, verbose=False):
    '''
    Calculates Chi2 vs. redshift for given PCA template.

    Args:
        redshifts: array of redshifts to evaluate
        targets : list of Target objects
        template: dictionary with keys
            - wave : array of restframe wavelengths [angstroms]
    ...
    Notes:
        template.flux is a basis set; spectra will be modelled as
    ...
    for t in targets:
        print("Scanning z for target: %d of %d" % (t.id, ntargets))
    ...
    for i, z in enumerate(redshifts):
        ...
        ## If all targets have the same number of spectra with the same wavelength grids, 
        ## we only need to calculate this once per redshift.
        ...
        for j in range(ntargets):
            ...
            ##  Length of flux - two fitting params (amplitude and redshift).
            dof           = len(flux) - 2
            
            print("Template type: %s and redshift: %.3lf; chi2: %.4le for %d degrees of freedom." % (template.type, z, zchi2[j,i], dof))

            ##  Penalize chi2 for negative [OII] flux; ad-hoc.
            if template.type in ['GALAXY', 'LBG']:
...
## DEBUG: duplicated code, but provide a direct way to fit a template to a set of spectra at a given redshift.
    ...
    for i, s in enumerate(spectra):
        ...
        if key not in Tx:
            ...
            for j in range(nbasis):
                Ti[:,j] = rebin.trapz_rebin((1. + z)*template.wave, template.flux[j], s.wave)
