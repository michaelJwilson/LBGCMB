...
class gTable(table_base):
    ...
    def write_FITS(self, overwrite=True):
        """
        ...
        if os.path.exists(self.filename+'.FITS') & overwrite:
...
ASCIItoFITS(infile, [comment='#'])
    ...
    thdulist.writeto(infile+'.FITS', overwrite=True)
...
class Readfile():
    ...
    def write_fits(self):
        ...
        #### Add modification time of "infile" to FITS header
        infile_mod_time = time.strftime("%m/%d/%Y %I:%M:%S %p", time.localtime(os.path.getmtime(self.filename)))
        
        ## ASTROPY DEPRECATED
        ## thdulist[1].header.update('MODTIME',  infile_mod_time)

        ## https://github.com/saltastro/polsalt/issues/44
        thdulist[1].header['MODTIME'] = infile_mod_time 

        thdulist.writeto(self.filename + '.FITS', overwrite=True)
